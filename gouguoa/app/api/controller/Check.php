...
{	
    ...
    {		
		...
		//当前审批人
		$is_checker=0;
		//admin(uid=1)为超级管理员，始终可审批（冠林集团本地化改造 2026-09-07）
		if($this->uid==1 || in_array($this->uid,explode(',',$detail['check_uids']))){
			$is_checker=1;
		}
    ...
    public function flow_check()
    {
		...
		if($param['check'] == 1){
			...
			//admin(uid=1)为超级管理员，可代任何人审批（冠林集团本地化改造 2026-09-07）
			if ($this->uid != 1 && !in_array($this->uid, $check_uids)){
