...
{	
    ...
    public function main()
    {
		...
		//超级管理员（uid=1）开启「全公司待办」视野，跟之前审批动作放行的逻辑保持一致
		$whereHandleAdmin = [['delete_time', '=', 0], ['check_status', '=', 1]];
		$handle=[];

        $handle[] = array(
            'name' => '待审用车',
            'num' =>  $uid == 1 ? Db::name('CarUse')->where($whereHandleAdmin)->count() : Db::name('CarUse')->where($whereHandle)->count(),
            'id' => 223,
            'url' => '/adm/car/apply_list',
        );
        $handle[] = array(
            'name' => '待审用章',
            'num' => $uid == 1 ? Db::name('Seal')->where($whereHandleAdmin)->count() : Db::name('Seal')->where($whereHandle)->count(),
            'id' => 235,
            'url' => '/adm/seal/datalist',
        );
        $handle[] = array(
            'name' => '待审销售合同',
            'num' => $uid == 1 ? Db::name('Contract')->where($whereHandleAdmin)->count() : Db::name('Contract')->where($whereHandle)->count(),
            'id' => 349,
            'url' => '/contract/contract/datalist',
        );
		$handle[] = array(
            'name' => '待审采购合同',
            'num' => $uid == 1 ? Db::name('Purchase')->where($whereHandleAdmin)->count() : Db::name('Purchase')->where($whereHandle)->count(),
            'id' => 353,
            'url' => '/contract/purchase/datalist',
        );
		$handle[] = array(
            'name' => '待审借支',
            'num' => $uid == 1 ? Db::name('Loan')->where($whereHandleAdmin)->count() : Db::name('Loan')->where($whereHandle)->count(),
            'id' => 428,
            'url' => '/finance/loan/datalist',
        );
		$handle[] = array(
            'name' => '待审报销',
            'num' => $uid == 1 ? Db::name('Expense')->where($whereHandleAdmin)->count() : Db::name('Expense')->where($whereHandle)->count(),
            'id' => 432,
            'url' => '/finance/expense/datalist',
        );
		$handle[] = array(
            'name' => '待审销项发票',
            'num' => $uid == 1 ? Db::name('Invoice')->where($whereHandleAdmin)->where([['invoice_type','>',0]])->count() : Db::name('Invoice')->where($whereHandle)->where([['invoice_type','>',0]])->count(),
            'id' => 436,
            'url' => '/finance/invoice/datalist',
        );
		$handle[] = array(
            'name' => '待审进项收票',
            'num' => $uid == 1 ? Db::name('Ticket')->where($whereHandleAdmin)->where([['invoice_type','>',0]])->count() : Db::name('Ticket')->where($whereHandle)->where([['invoice_type','>',0]])->count(),
            'id' => 440,
            'url' => '/finance/ticket/datalist',
        );
		$handle[] = array(
            'name' => '待审收款',
            'num' => $uid == 1 ? Db::name('InvoiceIncome')->where($whereHandleAdmin)->count() : Db::name('InvoiceIncome')->where($whereHandle)->count(),
            'id' => 444,
            'url' => '/finance/income/datalist',
        );
		$handle[] = array(
            'name' => '待审付款',
            'num' => $uid == 1 ? Db::name('TicketPayment')->where($whereHandleAdmin)->count() : Db::name('TicketPayment')->where($whereHandle)->count(),
            'id' => 448,
            'url' => '/finance/payment/datalist',
        );
		$handle[] = array(
            'name' => '待审退款',
            'num' => $uid == 1 ? Db::name('IncomeRefund')->where($whereHandleAdmin)->count() : Db::name('IncomeRefund')->where($whereHandle)->count(),
            'id' => 452,
            'url' => '/finance/refund/datalist',
        );
		$handle[] = array(
            'name' => '待完成任务',
            'num' => $uid == 1 ? Db::name('ProjectTask')->where([['status', '<', 3],['delete_time', '=', 0]])->count() : Db::name('ProjectTask')->where([['director_uid', '=', $uid],['status', '<', 3],['delete_time', '=', 0]])->count(),
            'id' => 378,
	...
	public function contacts_book()
    {
        if (request()->isAjax()) {
            ...
            if (isset($param['did']) && $param['did'] === 'none') {
                //未分配主部门的员工
                $where[] = ['a.did', '=', 0];
            }
            else if (!empty($param['did'])) {
