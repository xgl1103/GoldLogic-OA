...
class Captcha
{
    ...
    public function create(?string $config = null)
    {
        ...
        // 本地调试补丁：支持绝对路径字体（Windows 下 GD 的 imagettftext 无法打开含中文的路径，
        // 因此把字体复制到 C:/Code/oa-env/fonts 并在 config/captcha.php 中指定 fontttf 为绝对路径）
        if (!empty($this->fontttf) && preg_match('#^([A-Za-z]:)?[/\\\\]#', $this->fontttf)) {
            $fontttf = $this->fontttf;
        } else {
